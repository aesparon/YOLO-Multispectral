...
class Annotator:
    ...
    def box_label(self, box, label: str = "", color: tuple = (128, 128, 128), txt_color: tuple = (255, 255, 255)):
        ...
        multi_points = isinstance(box[0], (list, tuple))  # polygon: [[x,y],...]
        ...
        if multi_points:
            # sanitize polygon points
            pts = []
            for p in box:
                if not isinstance(p, (list, tuple)) or len(p) < 2:
                    continue
                x = self._safe_int(p[0])
                y = self._safe_int(p[1])
            ...
            if len(box) < 4:
                return
            x1 = self._safe_int(box[0]); y1 = self._safe_int(box[1])
            x2 = self._safe_int(box[2]); y2 = self._safe_int(box[3])
        ...
        if self.pil:
            if multi_points:
                self.draw.polygon(draw_box, width=self.lw, outline=color)
            else:
                self.draw.rectangle(draw_box, width=self.lw, outline=color)
            ...
            if multi_points:
                cv2.polylines(self.im, [np.asarray(draw_box, dtype=int)], True, color, self.lw)
            else:
                cv2.rectangle(self.im, p1, (draw_box[2], draw_box[3]), color, thickness=self.lw, lineType=cv2.LINE_AA)
