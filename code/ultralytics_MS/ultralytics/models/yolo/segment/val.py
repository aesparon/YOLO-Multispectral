...
class SegmentationValidator(DetectionValidator):
    ...
    def _prepare_pred(
        ...
        # --- YOLO-Multispectral fork: robust mask coeff slicing ---
        # Some patched/fork heads may return extra columns; the *mask coefficients* are the last proto_c channels.
        proto_c = int(proto.shape[0])  # usually 32
        masks_in = pred[:, 6:]

        # If the naive slice doesn't match proto channels, take the last proto_c columns
        if masks_in.shape[1] != proto_c:
            masks_in = pred[:, -proto_c:]

        if masks_in.shape[1] != proto_c:
            print(f"[VAL_MASK_FIX] pred shape={tuple(pred.shape)} proto={tuple(proto.shape)} using last {proto_c} coeffs")


        pred_masks = self.process(proto, masks_in, pred[:, :4], shape=pbatch["imgsz"])
