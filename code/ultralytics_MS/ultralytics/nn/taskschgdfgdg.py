...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # --- patched defaults for this fork (nc guard) ---
        # Some YAML configs in this fork may omit nc; ensure it exists before use.
        if "nc" not in self.yaml or self.yaml.get("nc", None) is None:
            self.yaml["nc"] = int(nc) if nc is not None else 80
        # --- end patched defaults ---
        if nc is not None and int(nc) != int(self.yaml["nc"]):
