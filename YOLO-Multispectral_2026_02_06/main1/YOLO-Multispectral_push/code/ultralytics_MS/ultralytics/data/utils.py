...
import cv2
# multi-channel modification -> multi_channels_mod.py
import tifffile
...
def verify_image_label(args: Tuple) -> List:
    ...
    #     # multi-channel modification -> multi_channels_mod.py        
    #     if im==None:
    #         if im_file.endswith('.tif') or im_file.endswith('.tiff'):
    #             # Check shape and dtype instead of verify()
    #             assert im is not None and im.size > 0, f"Empty image: {im_file}"
    #             shape = im.shape  # (H, W, C)

        # Verify images
        # andrew esparon - mod for multi band
        if im_file.endswith('.tif') or im_file.endswith('.tiff'):
