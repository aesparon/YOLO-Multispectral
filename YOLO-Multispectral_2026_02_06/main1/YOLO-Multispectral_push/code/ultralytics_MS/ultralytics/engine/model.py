...
class Model(torch.nn.Module):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        Notes (YOLO-Multispectral fork):
        - When a patched .pt is loaded (CBAM/ECA/SpectralConv/DropBlock/GroupNorm/Wavelet wrappers),
          do NOT rebuild the nn.Module graph from YAML during training, or the custom modules get dropped.
        - Ensure args['model'] is a valid string path for trainer initialization.
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and getattr(self.session.model, "id", None):  # HUB session with loaded model
            if any(kwargs):
        ...
        # --- YOLO-Multispectral fork: ensure args['model'] is always a string path ---
        if args.get("model") is None:
            args["model"] = (
                self.overrides.get("model")
                or self.model_name
                or self.ckpt_path
                or getattr(self.model, "pt_path", None)
                or getattr(self.model, "yaml_file", None)
            )
        if args.get("model") is None:
            raise TypeError("[TRAIN] args['model'] is None. Expected a model path string.")

        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks)

        if not args.get("resume"):  # manually set model only if not resuming
            # Preserve patched nn.Module graphs loaded from .pt (do not rebuild from YAML).
            if self.ckpt and isinstance(self.model, torch.nn.Module):
                self.trainer.model = self.model
                try:
                    self.trainer.model.args = args
                except Exception:
                    pass
            else:
