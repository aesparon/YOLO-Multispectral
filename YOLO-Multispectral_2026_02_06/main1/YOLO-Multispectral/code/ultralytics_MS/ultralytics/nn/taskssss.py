...
class BaseModel(torch.nn.Module):
    ...
    def fuse(self, verbose=True):
        """
        Disabled Conv+BN fusion for this fork.

        We keep the model in its non-fused form to avoid breaking custom modules
        (e.g. WaveletConv2d) and custom normalization. Ultralytics normally fuses
        Conv+BN for inference speed, but here we prefer stability / correctness.
        """
        from ultralytics.utils import LOGGER
        LOGGER.info("fuse() called – skipping Conv+BN fusion (WNN/multispectral fork).")
        # Mark as "fused" so is_fused() checks pass if they rely on internal flags.
        # If your BaseModel has its own fused flag, you can set it here, e.g.:
        # self._is_fused = True
        return self
...
def parse_model(d, ch, verbose=True):
    # --- patched defaults for this fork ---
    scale = ''  # patched default to avoid UnboundLocalError when YAML lacks scale/scales
    # Some YAML build paths in this fork produce nc=None; default to COCO to allow graph construction.
    # Training will overwrite heads to your real nc anyway.
    if isinstance(d, dict) and (d.get('nc', None) is None):
        d['nc'] = 80  # patched default nc for YAML builds
    # --- end patched defaults ---
    """
    ...
    scale = d.get("scale") or ""  # ensure defined even when YAML has no scales
    ...
    # --- multispectral fork guard: ensure nc is valid ---
    if nc is None:
        # Official Ultralytics model YAMLs always include nc.
        # In this fork, some YAML-based build paths (e.g. temporary YAMLs during checkpoint patching)
        # may omit it, which causes head init to crash. Default to COCO for graph construction.
        LOGGER.warning("Model YAML missing 'nc'. Defaulting to nc=80 for graph construction.")
        nc = 80
        d['nc'] = 80
    # --- end guard ---
